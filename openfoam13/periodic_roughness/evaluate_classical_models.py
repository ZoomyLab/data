...
def evaluate_classical_models(data):
    nut = data["nut"]  # [Samples, 50]
    eta = data["z"]  # [Samples, 50] (Strictly 0.0 to 1.0)
    h = data["h"][:, None]
    u_star = data["ustar"][:, None]

    # 1. Parabolic (Strictly evaluated on the dimensionless unit interval)
    # Physical Formula: nu_t = kappa * u_* * z * (1 - z/h)
    # Unit Formula (z = eta*h): nu_t = kappa * u_* * h * eta * (1 - eta)
    nut_para = 0.41 * u_star * h * eta * (1.0 - eta)

    # 2. Elder
    nut_elder = np.tile((0.41 / 6.0) * u_star * h, (1, nut.shape[1]))

    print(
        f"\n{'=' * 60}\nEVALUATING CLASSICAL RANS CLOSURES (ON UNIT INTERVAL)\n{'=' * 60}"
    )
    print(
        f"Data Sanity Check: Avg Depth (h) = {np.mean(h):.4f} m, Avg U_* = {np.mean(u_star):.4f} m/s"
    )

    print(f"1. Parabolic R^2 : {r2_score(nut.flatten(), nut_para.flatten()):+.4f}")
    print(f"2. Elder Model R^2: {r2_score(nut.flatten(), nut_elder.flatten()):+.4f}")
    print("=" * 60)
...
def plot_comparison(data):
    ...
    # ==========================================================
    # We now unpack dU_dx and dU_dy directly from the dataset!
    nut = data["nut"]
    md = data["md"]
    U = data["U"]
    dU_dy_all = data["dU_dy"]  # <--- Load native dU_dy
    dU_dx_all = data["dU_dx"]  # <--- Load native dU_dx
    eta = data["z"]
    h = data["h"]
    u_star = data["ustar"]
    ...
    U_true = U[idx]
    dU_dy = dU_dy_all[idx]  # <--- Use exact OpenFOAM vertical gradient
    dU_dx = dU_dx_all[idx]  # <--- Use exact OpenFOAM horizontal gradient
    h_val = h[idx]
    ...
    # ==========================================================
    # (Notice we deleted the np.gradient line. We use the true arrays directly!)
    ...
    c1, c2, c3, c4 = 0.144913, -0.334451, 79.2029, -0.54807

    ml_term1 = c1 * (nut_true * np.abs(dU_dy))
    ml_term2 = c2 * (nut_true * np.abs(dU_dx))
